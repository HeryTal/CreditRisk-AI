...
function LiveScore({ formData }) {
  ...
  const getScoreColor = (currentScore) => {
    if (currentScore >= 80) return { bg: 'bg-green-500', text: 'text-green-400', label: 'Excellent' }
    if (currentScore >= 65) return { bg: 'bg-emerald-500', text: 'text-emerald-400', label: 'Good' }
    if (currentScore >= 50) return { bg: 'bg-yellow-500', text: 'text-yellow-400', label: 'Average' }
    if (currentScore >= 35) return { bg: 'bg-orange-500', text: 'text-orange-400', label: 'Low' }
    ...
    `}>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
        <div className="flex items-center gap-1">
          ...
          <span className="text-gray-300">{formData.age} years</span>
        </div>
        <div className="flex items-center gap-1">
          <i className="fas fa-check-circle text-green-500/50 text-[10px]" />
          <span className="text-gray-500">Amount: </span>
          <span className="text-gray-300">{Number(formData.credit_amount).toLocaleString()} EUR</span>
        </div>
        <div className="flex items-center gap-1">
          <i className="fas fa-check-circle text-green-500/50 text-[10px]" />
          <span className="text-gray-500">Duration: </span>
          <span className="text-gray-300">{formData.duration} months</span>
        </div>
        <div className="flex items-center gap-1">
          <i className="fas fa-check-circle text-green-500/50 text-[10px]" />
