...
function PredictionResult({ predictionData, onReset }) {
  ...
  const getRiskLevel = (score) => {
    if (score < 30) return { label: 'Very low', color: 'green', icon: 'fa-shield-alt' }
    if (score < 50) return { label: 'Low', color: 'emerald', icon: 'fa-check-circle' }
    if (score < 70) return { label: 'Moderate', color: 'yellow', icon: 'fa-exclamation-triangle' }
    if (score < 85) return { label: 'High', color: 'orange', icon: 'fa-exclamation-circle' }
    return { label: 'Very high', color: 'red', icon: 'fa-times-circle' }
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                AI prediction - {new Date().toLocaleDateString()}
              </p>
            </div>
          </div>
          ...
          `}>
            <div className="flex items-center gap-2">
              ...
              <span className={`
                ...
                ${isGoodPrediction ? 'text-green-400' : 'text-red-400'}
              `}>
                {isGoodPrediction ? 'LOW RISK' : 'HIGH RISK'}
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-400 leading-relaxed">
                {isGoodPrediction
                  ? 'This client has a low default risk. The loan can be approved under standard terms.'
                  : 'This client has a high risk. Recommendation: reject the loan or request additional guarantees.'}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="mb-6">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium text-white">Influence factors</span>
              </div>
